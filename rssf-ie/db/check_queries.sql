...
SELECT MB.TEAM1, MB.TEAM2, MB.COMPETITION, MB.PHASE, PB.NUM
FROM 
(MATCH_PAIR MA JOIN PHASE PA ON MA.COMPETITION = PA.COMPETITION AND MA.PHASE = PA.NAME),
(MATCH_PAIR MB JOIN PHASE PB ON MB.COMPETITION = PB.COMPETITION AND MB.PHASE = PB.NAME) 
WHERE MA.COMPETITION = MB.COMPETITION
AND PB.NUM > PA.NUM
AND (MB.TEAM1 = MA.TEAM2 OR MB.TEAM2 = MA.TEAM2)
ORDER BY MB.COMPETITION, PB.NUM 
...
and T1.COUNTRY IS NOT NULL AND T2.COUNTRY IS NULL